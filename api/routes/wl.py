...
from api.models.nm_aquifer_models import WaterLevels, WaterLevelsContinuous_Pressure, WaterLevelsContinuous_Acoustic, \
    Location
...
@router.get("/waterlevels", response_model=List[nm_aquifer_schemas.WaterLevels], tags=['Groundwater Levels'])
def read_waterlevels(pointid: str = None, limit: int = 100, db: Session = Depends(get_nm_aquifer)):
    return _read(db, WaterLevels, limit)


@router.get("/waterlevelspressure", response_model=List[nm_aquifer_schemas.WaterLevelsPressure],
...
@router.get("/waterlevelsacoustic", response_model=List[nm_aquifer_schemas.WaterLevelsAcoustic],
            tags=['Groundwater Levels'])
def read_waterlevelspressure(pointid: str = None, limit: int = 100, db: Session = Depends(get_nm_aquifer)):
    return _read(db, WaterLevelsContinuous_Acoustic, limit)


@router.get("/locations", response_model=List[nm_aquifer_schemas.Location], tags=['Locations'])
